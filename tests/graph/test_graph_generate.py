...
def patch_system():
    ...
    # --- [Arrange] 事前データの準備 ---
    sys.add_equipment(EquipmentInstance(EQ_VO, "Vo.Mic", NodeType.MIC))
    sys.add_equipment(EquipmentInstance(EQ_LG, "LG.Amp", NodeType.INSTRUMENT))
    sys.add_equipment(EquipmentInstance(EQ_LG_MIC, "LG.Mic", NodeType.MIC))
    sys.add_equipment(EquipmentInstance(EQ_SB, "StageBox16", NodeType.STAGE_BOX))
    sys.add_equipment(EquipmentInstance(EQ_MIX, "MG24/14FX Console", NodeType.MIXER))

    # 楽器ポート
    sys.add_port(PortInstance(VO_OUT, "Out", PortDirection.OUT, PortGender.MALE, EQ_VO))
    sys.add_port(PortInstance(LG_OUT, "Out", PortDirection.OUT, PortGender.MALE, EQ_LG))
    sys.add_port(
